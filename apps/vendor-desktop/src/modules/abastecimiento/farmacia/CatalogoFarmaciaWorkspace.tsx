    ...
    <section className="flex min-h-0 flex-1 gap-2">
      <div className="flex flex-[40] shrink-0 flex-col overflow-hidden rounded-[28px] border border-[#1E88C7]/50 bg-[#FDFCF9]">
        <div className="shrink-0 flex h-[42px] items-center justify-between gap-2 px-4 border-b bg-[#E3F1FA]/60 border-[#1E88C7]/15">
          <div className="flex items-center gap-2">
            <Pill size={13} strokeWidth={2} className="shrink-0 text-[#1E88C7]" />
        ...
        <div className="flex flex-col flex-1 overflow-hidden">
          ...
          {!catalogo.creandoAbierto && (
            <div className="shrink-0 px-4 pb-4 pt-2 flex gap-2">
              <button
                type="button"
                disabled={catalogo.termino.trim().length === 0 && catalogo.resultados.length === 0 || catalogo.productoSeleccionado !== null}
              ...
              >
                ...
                <kbd className="pointer-events-none absolute -top-7 left-1/2 -translate-x-1/2 whitespace-nowrap rounded border border-[#fef08a] bg-[#fefce8] px-2 py-1 text-[11px] font-bold leading-none text-[#713f12] opacity-0 transition-opacity duration-150 group-hover:opacity-100 z-10">Ctrl+Enter</kbd>
              </button>
            </div>
          )}
        </div>
      ...
      <div className="flex flex-[60] min-h-0 flex-col overflow-hidden rounded-[28px] border border-[#1E88C7]/30 bg-[#FDFCF9]">
        <div className="shrink-0 flex h-[42px] items-center gap-2 px-4 border-b bg-[#E3F1FA]/60 border-[#1E88C7]/15">
          {catalogo.creandoAbierto ? (
            <X size={13} strokeWidth={2} className="shrink-0 text-[#1E88C7]" />
          ) : (
            <BookOpen size={13} strokeWidth={2} className="shrink-0 text-[#1E88C7]" />
          )}
          <span className="text-[13px] font-semibold uppercase tracking-tight leading-none text-[#121416]">
            {catalogo.creandoAbierto ? 'NUEVO PRODUCTO' : modoDetalle === 'corrigiendo' ? 'CORREGIR DATOS BÁSICOS PRODUCTO' : modoDetalle === 'desactivando' ? 'DESACTIVAR PRODUCTO CATÁLOGO' : catalogo.vistaActiva === 'detalle' ? 'FICHA PRODUCTO' : catalogo.vistaActiva === 'presentaciones' ? 'PRESENTACIONES PRODUCTO' : catalogo.vistaActiva === 'precios' ? 'PRECIOS PRODUCTO' : 'FICHA PRODUCTO'}
          </span>
        </div>
        ...
        <div className="flex-1 min-h-0 flex flex-col">
          ...
          {!catalogo.creandoAbierto && catalogo.productoSeleccionado === null && catalogo.productoPreview === null && !catalogo.sinResultados && (
            <div className="flex flex-col items-center justify-center pb-14 pt-[106px]">
              <BookOpen size={32} className="text-[#1E88C7]/30" />
              <p className="mt-3 text-center text-[12px] text-slate-400">
                Selecciona un producto en los resultados de la búsqueda para ver su ficha
              </p>
            </div>
          )}
        </div>
