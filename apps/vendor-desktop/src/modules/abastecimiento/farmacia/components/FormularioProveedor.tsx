...
export function FormularioProveedor({
  ...
  return (
    <form onSubmit={(event: FormEvent<HTMLFormElement>) => void guardar(event)} className="rounded-2xl border border-[#E3F1FA] bg-white p-5">
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        <label className="space-y-1">
          ...
          <input className="h-11 w-full rounded-xl border border-[var(--dv-input-border)] px-3" value={razonSocial} onChange={(event: ChangeEvent<HTMLInputElement>) => setRazonSocial(event.target.value)} />
        </label>
        <label className="space-y-1">
          <span className="text-[11px] font-bold uppercase text-slate-500">RUC</span>
          <input className="h-11 w-full rounded-xl border border-[var(--dv-input-border)] px-3" value={ruc} onChange={(event: ChangeEvent<HTMLInputElement>) => setRuc(event.target.value)} />
        </label>
        <label className="space-y-1">
          <span className="text-[11px] font-bold uppercase text-slate-500">Contacto</span>
          <input className="h-11 w-full rounded-xl border border-[var(--dv-input-border)] px-3" value={nombreContacto} onChange={(event: ChangeEvent<HTMLInputElement>) => setNombreContacto(event.target.value)} />
        </label>
        <label className="space-y-1">
          <span className="text-[11px] font-bold uppercase text-slate-500">Teléfono</span>
          <input className="h-11 w-full rounded-xl border border-[var(--dv-input-border)] px-3" value={telefono} onChange={(event: ChangeEvent<HTMLInputElement>) => setTelefono(event.target.value)} />
        </label>
        <label className="space-y-1 md:col-span-2">
          <span className="text-[11px] font-bold uppercase text-slate-500">Condiciones de pago</span>
          <input className="h-11 w-full rounded-xl border border-[var(--dv-input-border)] px-3" value={condicionesPago} onChange={(event: ChangeEvent<HTMLInputElement>) => setCondicionesPago(event.target.value)} />
        </label>
      </div>
