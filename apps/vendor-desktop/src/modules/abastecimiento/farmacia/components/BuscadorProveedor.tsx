...
function chipEstado(estado: string): string {
  return estado === 'ACTIVO' ? 'bg-[#E8F0E6] text-[#3B6B34]' : 'bg-red-50 text-red-600'
}
    ...
    <section className="flex flex-1 items-start justify-center px-6 py-14">
      <div className="w-full max-w-3xl">
        <div className="relative">
          <Search className="absolute left-5 top-1/2 h-5 w-5 -translate-y-1/2 text-[#3B6B34]" />
          <input
            ref={inputRef}
            value={termino}
            ...
            className="h-16 w-full rounded-[28px] border border-[#E8F0E6] bg-white pl-14 pr-6 text-[18px] font-semibold text-slate-800 outline-none focus:border-[#3B6B34] focus:ring-4 focus:ring-[#E8F0E6]"
          />
        </div>
        ...
        {cargando && puedeMostrarResultados && (
          <p className="mt-5 text-center text-[13px] font-semibold text-[#3B6B34]">Buscando...</p>
        )}
        {!cargando && puedeMostrarResultados && resultados.length > 0 && (
          <div className="mt-5 overflow-hidden rounded-2xl border border-[#E8F0E6] bg-white">
            {resultados.map((proveedor) => (
              <button
                key={proveedor.id}
                type="button"
                onClick={() => onSeleccionar(proveedor)}
                className="block w-full border-b border-[#E8F0E6] px-5 py-4 text-left hover:bg-[#E8F0E6]"
              >
            ...
            <button
              type="button"
              onClick={onIrManual}
              className="flex w-full items-center gap-3 px-5 py-4 text-left text-[13px] font-bold text-[#3B6B34] hover:bg-[#E8F0E6]"
            >
        ...
        {!cargando && puedeMostrarResultados && resultados.length === 0 && (
          <div className="mt-5 rounded-2xl border border-[#E8F0E6] bg-white p-5 text-center">
