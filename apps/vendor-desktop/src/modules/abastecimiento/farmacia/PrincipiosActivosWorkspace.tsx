...
function ItemLista({ principio, seleccionado, onSeleccionar }: ItemListaProps): ReactElement {
  return (
    <button
      ...
      className={`block w-full border-b border-[#E0F2FE] px-3 py-2 text-left transition last:border-0 ${seleccionado ? 'bg-[#E0F2FE]' : 'hover:bg-[#E0F2FE]'}`}
    >
      ...
      <div className="mt-0.5 flex items-center gap-2">
        ...
        {principio.esCombinacion && (
          <span className="shrink-0 rounded-full bg-[#E0F2FE] px-1.5 py-0.5 text-[9px] font-bold text-[#0284C7]">COMBINACIÓN</span>
        )}
  ...
  return (
    <section className="flex min-h-0 flex-1 flex-col">
      <div className="flex-1 overflow-auto px-5 py-4 space-y-4">
        {cargando && <p className="text-[12px] font-semibold text-[#0284C7]">Cargando...</p>}
        {!cargando && (
          <>
            <div className="flex flex-wrap gap-2">
              <BadgeCondicion condicion={principio.condicionVentaIfa} />
              {principio.esCombinacion && (
                <span className="rounded-full bg-[#E0F2FE] px-2 py-0.5 text-[10px] font-bold text-[#0284C7]">COMBINACIÓN DE IFAs</span>
              )}
              ...
              <div>
                ...
                <div className="space-y-1">
                  {principio.productosVinculados.map((producto) => (
                    <div key={producto.id} className="flex items-center justify-between rounded-xl border border-[#E0F2FE] bg-white px-3 py-2">
      ...
      <div className="flex-1 overflow-auto px-5 py-4 space-y-3">
        ...
        <label className="block">
          ...
          <input
            ...
            className="mt-1 h-[36px] w-full rounded-xl border border-[#E0F2FE] bg-white px-3 text-[13px] font-semibold text-slate-800 outline-none focus:border-[#0284C7] focus:ring-2 focus:ring-[#E0F2FE]"
          />
        </label>
        ...
        <label className="block">
          ...
          <select
            ...
            className="mt-1 h-[36px] w-full rounded-xl border border-[#E0F2FE] bg-white px-3 text-[13px] font-semibold text-slate-800 outline-none focus:border-[#0284C7]"
          >
        ...
        <label className="block">
          ...
          <textarea
            ...
            className="mt-1 w-full rounded-xl border border-[#E0F2FE] bg-white px-3 py-2 text-[12px] text-slate-800 outline-none focus:border-[#0284C7] focus:ring-2 focus:ring-[#E0F2FE] resize-none"
          />
        </label>

        <div className="flex flex-col gap-2 rounded-xl border border-[#E0F2FE] bg-white px-4 py-3">
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              className="h-4 w-4 rounded border-[#E0F2FE] text-[#0284C7]"
            />
      ...
      <div className="flex flex-[40] shrink-0 flex-col overflow-hidden rounded-[28px] border border-[#0284C7]/50 bg-[#FDFCF9]">
        <div className="shrink-0 flex h-[42px] items-center justify-between gap-2 px-4 border-b bg-[#E0F2FE]/60 border-[#0284C7]/15">
          <div className="flex items-center gap-2">
            <FlaskConical size={13} strokeWidth={2} className="shrink-0 text-[#0284C7]" />
        ...
        <div className="flex flex-col flex-1 overflow-hidden">
          <div className="px-3 pt-3">
            ...
            {estado.cargando && (
              <p className="mt-2 text-[11px] font-semibold text-[#0284C7]">Buscando...</p>
            )}
          </div>
          ...
          <div className="flex-1 overflow-auto mt-2">
            {!estado.cargando && estado.resultados.length === 0 && (
              <div className="flex flex-col items-center justify-center py-10">
                <FlaskConical size={24} className="text-[#0284C7]/30" />
                <p className="mt-2 text-center text-[11px] text-slate-400">Sin resultados</p>
              </div>
            )}
            {estado.resultados.length > 0 && (
              <div className="overflow-hidden rounded-xl border border-[#E0F2FE] bg-white mx-3">
        ...
        <div className="flex-1 min-h-0 flex flex-col">
          ...
          {estado.modo === 'busqueda' && (
            <div className="flex flex-col items-center justify-center pb-14 pt-[106px]">
              <FlaskConical size={32} className="text-[#0284C7]/30" />
