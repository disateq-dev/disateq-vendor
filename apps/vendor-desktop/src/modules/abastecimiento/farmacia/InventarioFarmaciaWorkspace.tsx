...
export function InventarioFarmaciaWorkspace(): ReactElement {
  ...
  return (
    <section className="flex min-h-0 flex-1 gap-2">
      <div className="flex flex-[35] shrink-0 flex-col overflow-hidden rounded-[28px] border border-[#1E88C7]/50 bg-[#FDFCF9]">
        <div className="shrink-0 flex h-[42px] items-center gap-2 px-4 border-b bg-[#E3F1FA]/60 border-[#1E88C7]/15">
          <Boxes size={13} strokeWidth={2} className="shrink-0 text-[#1E88C7]" />
        ...
        <div className="px-3 pt-3 pb-2 shrink-0">
          <input
            ...
            className="w-full h-9 rounded-xl border border-[#E3F1FA] bg-white px-3 text-[12px] font-semibold text-slate-700 outline-none focus:border-[#1E88C7]"
          />
        </div>
        <div className="min-h-0 flex-1 overflow-auto">
          {cargando && <div className="px-4 py-6 text-[13px] font-semibold text-[#1E88C7]">Cargando...</div>}
          ...
          {!cargando && !error && inventarioFiltrado.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full text-left px-4 py-3 border-b border-[#E3F1FA] transition ${
                  item.presentacionId === seleccionado?.presentacionId ? 'bg-[#E3F1FA]/40' : ''
                }`}
              >
      ...
      <div className="flex min-h-0 flex-[65] flex-col overflow-hidden rounded-[28px] border border-[#1E88C7]/30 bg-[#FDFCF9]">
        <div className="shrink-0 flex h-[42px] items-center gap-2 px-4 border-b bg-[#E3F1FA]/60 border-[#1E88C7]/15">
          <Package size={13} strokeWidth={2} className="shrink-0 text-[#1E88C7]" />
        ...
        <div className="min-h-0 flex-1 flex flex-col overflow-auto">
          {seleccionado === null && (
            <div className="flex flex-1 flex-col items-center justify-center gap-3 p-8 text-center">
              <span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#E3F1FA] text-[#1E88C7]">
                <Package size={28} />
              </span>
            ...
            <div className="px-6 py-5 space-y-5">
              <div className="rounded-2xl border border-[#E3F1FA] bg-white p-5">
                ...
                <div className="mt-3 flex items-center gap-3">
                  <span className="text-[22px] font-bold text-[#1E88C7]">{seleccionado.totalDisponible.toFixed(0)}</span>
              ...
              <div className="rounded-xl border border-[#E3F1FA] bg-white p-4">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="h-9 w-24 rounded-lg border border-[#E3F1FA] px-3 text-[13px] font-semibold text-slate-700 outline-none focus:border-[#1E88C7]"
                  />
              ...
              {seleccionado.requiereLote && (
                <div className="rounded-2xl border border-[#E3F1FA] bg-white p-4">
                  <h3 className="text-[12px] font-bold uppercase tracking-widest text-slate-400 mb-3">LOTES VIGENTES</h3>
                  {cargandoLotes && <p className="text-[12px] font-semibold text-[#1E88C7]">Cargando lotes...</p>}
                  ...
                  {!cargandoLotes && lotes.length > 0 && lotes.map((lote) => {
                    ...
                    return (
                      <div key={lote.id} className="py-2 border-b border-[#E3F1FA] last:border-0">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <span className="text-[13px] font-bold text-[#1E88C7]">
