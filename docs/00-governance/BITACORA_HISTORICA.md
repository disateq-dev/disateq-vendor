# BITÁCORA HISTÓRICA — DISATEQ Vendor™

Archivo de decisiones ya **CONSOLIDADAS** o **RESUELTAS**, movidas aquí desde
`BITACORA_DECISIONES.md` el 2026-06-21 para reducir el costo de lectura de
la bitácora activa (que crecía sin límite por ser append-only). Nada se
elimina — esto es exactamente lo que pide `philosophy/EVOLUTION_AND_CONSOLIDATION.md`:
preservar el razonamiento histórico, no acumularlo por defecto en cada lectura.

**No leer este archivo de forma rutinaria.** Consultar solo cuando una
auditoría futura necesite el razonamiento completo detrás de una decisión ya
cerrada, o cuando se sospeche que un hallazgo "nuevo" ya fue resuelto antes
(evita reabrir como hallazgo nuevo algo que ya tiene contexto aquí).

Las decisiones aún abiertas (PENDIENTE DE IMPLEMENTAR / A EVALUAR) permanecen
en `BITACORA_DECISIONES.md`, no aquí.
...
**Estado:** CONSOLIDADO — doctrina irrevocable. **Nota 2026-06-21:** el
contenido de SOLID/Clean Code/normalización de esta entrada quedó formalizado
y ampliado en `docs/00-governance/ESTANDARES_TECNICOS.md` — esa es ahora la
referencia viva, esta entrada queda como registro histórico de origen.
...
corregir antes de conectarla a cualquier pantalla futura.
...
(timestamps + lectura línea por línea) antes de aprobar commit.
...
primer caso de uso real. **Nota:** la decisión pendiente de si adoptar
SheetWork en ABASTECIMIENTO FARMACIA vive activa en CURRENT_CONTEXT.md →
Próxima ventana de trabajo, no se repite aquí.
...
consulten `ESTANDARES_TECNICOS.md` como parte del protocolo normal.
